...
def test_get_aggregates_request_failures(client, app_module, monkeypatch, exc, expected_prefix):
    class FakeAsyncClient:
        def __init__(self, timeout):
...
def test_get_aggregates_http_status_error(client, app_module, monkeypatch):
    ...
    error = httpx.HTTPStatusError('upstream failed', request=request, response=bad_response)
